...
Building this mini blog end-to-end — from a hand-written Vite/TypeScript config through typed props, conditional styling, and a custom HOC — tied together concepts that felt separate while learning them individually in weeks 1–3. Typing the `Post` interface early and threading it through `posts.ts` → `PostList` → `Post` made it obvious how much TypeScript catches at compile time (mismatched prop shapes, typos in field names) that would otherwise surface as runtime bugs. Writing `withLogger` was the most valuable exercise: seeing how a HOC wraps a component to add behavior (logging) without touching its internals clarified *why* the pattern exists, beyond just knowing the syntax. Going forward, exploring `useMemo`/`useCallback` alongside `React.memo`, and a real component library, would be a natural next step.
